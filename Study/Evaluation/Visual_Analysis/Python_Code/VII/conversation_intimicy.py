...
def plot_pci(input_file_a, input_file_b, output_file_boxplot):
		...
		for row in csv_reader_a:
			...
			for subject in total_a:
				if row['subjectID'] == subject['subjectID']:
					subject['totalPci'] += float(row['pci_value'])
	...
	ax.set_ylabel('1 = low intimicy - 7 = high intimicy')
	ax.set_title('(i) Perceived Conversation Intimicy', fontweight= 'bold', fontsize=14)

	plt.savefig(output_file_boxplot)

	plt.show()

plot_pci(csv_survey_a, csv_survey_b, 'plot_VII_a_b.png')