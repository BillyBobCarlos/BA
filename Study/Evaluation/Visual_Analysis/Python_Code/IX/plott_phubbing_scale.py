...
def plot_reactance(input_file_a, input_file_b, output_file_boxplot):
		...
		for row in csv_reader_a:
			...
			for subject in total_a:
				if row['subjectID'] == subject['subjectID']:
					subject['totalPS'] += float(row['ps_value'])
	...
	plot_a = [(subject['totalPS']/subject['sureveyCounter']) for subject in total_a]
	plot_b = [(subject['totalPS']/subject['sureveyCounter']) for subject in total_b]
	...
	ax.set_ylabel('1 = low phubbing - 5 = high phubbing')
	ax.set_title('(m) Self-Perceived Phubbing Behavior\n(Phubbing Scale)', fontweight= 'bold', fontsize=14)

	plt.savefig(output_file_boxplot)

	plt.show()

plot_reactance(csv_survey_a, csv_survey_b, 'plot_IX_a_b.png')
